...
const typeDefs = gql`
  ...
  # In curly-brackets ({}), we specify the fields that type will return 
  # (Our User type will return three fields: firstName, lastName, and email. All 
  # three are strings and required).
  ...
  type Query {
    ...
    queryUsers(count: Int): [User]!
  }

  # Mutations must be in their own type
  type Mutation {
    # We are creating a mutation called "addUser" that takes in 3 arguments
    # These arguments will be available to our resolver, which will push the new user 
    #to the "users" array
...
const resolvers = {
    ...
    Query: {
      ...
      queryUsers: (_,arg) => users.slice(0,arg.count)
    },
...
const server = new ApolloServer({
  ...
  server.listen({ port: 8081 }).then(({ url }) => console.log(`GraphQL server running at ${url}`));
